...
- ✅ **2026-07-01**: "📖 Story Illustration" upgraded to **project-level management + Prompt detail enhancement**
  - **Project management**: entering the page shows a project list (cards: thumbnail / title / stage dots / updated time); click to enter a workspace. 4 stages (story / assets / storyboard / generate) are switchable, auto-persist to `output/projects/{id}/`, and restore on reopen. Same project can generate videos multiple times with side-by-side version comparison
  - **Prompt upgrade** (referencing waoowaoo film workflow): ① asset extraction now includes visual specs (characters: hair/face/outfit/identifying marks; scenes: space/material/lighting/atmosphere; props: structure/material/decoration); ② dedicated character 4-view reference sheet template (style consistency + missing-part completion + neutral background); ③ storyboard output adds shot_type/camera_move/lighting fields + continuity rules + asset whitelist; ④ new picture-book-specific panel image prompt (enforces reference consistency + photography rules + no text in image + single frame)
  - **Fix: resume losing reference images** — `persistence._frame_to_dict` missed serializing `reference_image_paths`, so img2img consistency references were lost on resume. Now fixed (including new shot_type/camera_move/lighting fields)
  - **Legacy task archiving**: history page distinguishes new projects (📁 editable) from legacy flat tasks (📜 read-only), without breaking old data
